...
// ../../node_modules/.pnpm/drizzle-orm@0.45.2_@neondatabase+serverless@0.10.4_@types+pg@8.20.0_pg@8.20.0/node_modules/drizzle-orm/neon-http/session.js
var rawQueryConfig = {
  arrayMode: false,
  fullResults: true
};
...
// ../../node_modules/.pnpm/drizzle-orm@0.45.2_@neondatabase+serverless@0.10.4_@types+pg@8.20.0_pg@8.20.0/node_modules/drizzle-orm/neon-http/driver.js
...
function construct(client, config = {}) {
  ...
  const driver = new NeonHttpDriver(client, dialect, { logger: logger3, cache: config.cache });
...
function numberColumnToSchema(column, z4, coerce2) {
  ...
  let schema = coerce2 === true || coerce2?.number ? integer2 ? z4.coerce.number() : z4.coerce.number().int() : integer2 ? z4.int() : z4.number();
  schema = schema.gte(min).lte(max);
  return schema;
}
function bigintColumnToSchema(column, z4, coerce2) {
  const unsigned = column.getSQLType().includes("unsigned");
  const min = unsigned ? 0n : CONSTANTS.INT64_MIN;
  const max = unsigned ? CONSTANTS.INT64_UNSIGNED_MAX : CONSTANTS.INT64_MAX;
  const schema = coerce2 === true || coerce2?.bigint ? z4.coerce.bigint() : z4.bigint();
  return schema.gte(min).lte(max);
}
function stringColumnToSchema(column, z4, coerce2) {
  ...
  let schema = coerce2 === true || coerce2?.string ? z4.coerce.string() : z4.string();
...
async function sendViaBrevo(to2, subject, html, fromName = "Matiyane Gas Distributors") {
  const BREVO_API_KEY = define_process_env_default.BREVO_API_KEY;
  const BREVO_SENDER_EMAIL = define_process_env_default.BREVO_SENDER_EMAIL || "noreply@matiyanegas.co.za";
...
async function sendViaSmtp(to2, subject, html, fromLabel) {
  const SMTP_HOST = define_process_env_default.SMTP_HOST;
  const SMTP_PORT = parseInt(define_process_env_default.SMTP_PORT || "587");
  const SMTP_USER = define_process_env_default.SMTP_USER;
  const SMTP_PASS = define_process_env_default.SMTP_PASS;
...
async function sendEmail(to2, subject, html, fromLabel = "Matiyane Gas Distributors") {
  if (define_process_env_default.BREVO_API_KEY) {
    await sendViaBrevo(to2, subject, html, fromLabel);
  } else {
    await sendViaSmtp(to2, subject, html, fromLabel);
  }
}
...
// src/lib/hubspot.ts
var HUBSPOT_API_KEY = define_process_env_default["HUBSPOT_API_KEY"];
...
function createOrdersRouter(db) {
  ...
  router.post("/orders", async (c) => {
    ...
    try {
      const [newOrder] = await db.insert(ordersTable).values({ orderRef, fullName, phone, email, deliveryAddress, suburb, specialInstructions: specialInstructions ?? null, status: "pending", totalAmount: totalAmount.toFixed(2), deliveryFee: deliveryFee.toFixed(2) }).returning();
      const insertedItems = await db.insert(orderItemsTable).values(orderItems.map((item) => ({ orderId: newOrder.id, productId: item.productId, productName: item.productName, quantity: item.quantity, unitPrice: item.unitPrice.toFixed(2), subtotal: item.subtotal.toFixed(2) }))).returning();
  ...
  router.get("/orders/:id/payfast", async (c) => {
    ...
    try {
      const [order] = await db.select().from(ordersTable).where(eq(ordersTable.id, params.data.id));
      if (!order) return c.json({ error: "Order not found" }, 404);
      const isSandbox = define_process_env_default["PAYFAST_SANDBOX"] === "true" || false;
      const merchantId = define_process_env_default["PAYFAST_MERCHANT_ID"] || "10000100";
      const merchantKey = define_process_env_default["PAYFAST_MERCHANT_KEY"] || "46f0cd694581a";
      const domains = define_process_env_default["REPLIT_DOMAINS"]?.split(",")[0];
      const baseUrl = domains ? `https://${domains}` : define_process_env_default["PUBLIC_URL"] || "http://localhost:80";
  ...
  router.get("/orders/:id/paystack", async (c) => {
    ...
    try {
      ...
      if (!order) return c.json({ error: "Order not found" }, 404);
      const publicKey = define_process_env_default["PAYSTACK_PUBLIC_KEY"] || "pk_test_xxxxxx";
      const domains = define_process_env_default["REPLIT_DOMAINS"]?.split(",")[0];
      const baseUrl = domains ? `https://${domains}` : define_process_env_default["PUBLIC_URL"] || "http://localhost:80";
  ...
  router.post("/payfast/notify", async (c) => {
    ...
    if (payment_status === "COMPLETE" && m_payment_id) {
      try {
        await db.update(ordersTable).set({ status: "paid", updatedAt: /* @__PURE__ */ new Date() }).where(eq(ordersTable.orderRef, m_payment_id));
  ...
  router.post("/paystack/webhook", async (c) => {
    ...
    if (event.event === "charge.success" && event.data?.reference) {
      ...
      try {
        await db.update(ordersTable).set({ status: "paid", updatedAt: /* @__PURE__ */ new Date() }).where(eq(ordersTable.orderRef, orderRef));
...
// src/lib/auth.ts
function getSecret() {
  return define_process_env_default["SESSION_SECRET"] || "matiyane-admin-secret-fallback";
}
...
function createAdminRouter(db) {
  ...
  router.post("/admin/login", async (c) => {
    ...
    const schema = z3.object({ password: z3.string() });
  ...
  router.get("/admin/orders", requireAdmin, async (c) => {
    ...
    try {
      ...
      const items = orderIds.length ? await db.select().from(orderItemsTable).where(sql`${orderItemsTable.orderId} = ANY(${sql.raw(`ARRAY[${orderIds.join(",")}]::int[]`)})`) : [];
  ...
  router.patch("/admin/orders/:id/status", requireAdmin, async (c) => {
    ...
    const schema = z3.object({
      status: z3.enum(["pending", "confirmed", "out_for_delivery", "delivered", "paid", "cancelled"])
    });
    const parsed = schema.safeParse(body);
    if (!parsed.success) {
      return c.json({ error: "Invalid status" }, 400);
  ...
  router.get("/admin/contacts", requireAdmin, async (c) => {
    ...
    try {
      ...
      const contacts = await db.select().from(contactMessagesTable).orderBy(desc(contactMessagesTable.createdAt)).limit(limit).offset(offset);
      return c.json({
...
var onRequest = async (context) => {
  const sql2 = Xs(context.env.DATABASE_URL);
